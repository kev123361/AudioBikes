...
public class CarMoveAndRightTurnBrake : MonoBehaviour
{
    ...
    public enum DriveState {Waiting, Driving, Turning, Forward, Brake };
    ...
    void Update()
    {
        ...
        if (state == DriveState.Waiting && distance <= 48f)
        {
        ...
        else if (state == DriveState.Forward)
        {
            moveTime += Time.deltaTime;
            if (moveTime >= 2.5f)
            {
        ...
        else if (state == DriveState.Brake)
        {
            if (!played) {
                audioSource.Play();
                played = true;
